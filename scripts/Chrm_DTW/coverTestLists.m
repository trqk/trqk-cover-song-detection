function [R,S,T,C] = coverTestLists(qlist, tlist, pwr, metric, verb, take1, pre_proc)
...
for q = 1 : nqlist            ## q list loop (songs to match)
  qline = qlist{q};
  if (verb > 0)
    slpos = index(qline, '/') + 1;
    dtpos = rindex(qline, '.');
    disp([datestr(rem(now, 1), 'HH:MM:SS'), ' - song ', num2str(q), ' - ', substr(qline, slpos, dtpos-slpos)]);
  end
  Q = load(qline);
  if pre_proc == false
    Q.F = chromnorm(chrompwr(Q.F, pwr));      % take sqrt of chroma vals
    Q.F = conj(Q.F);
  end
  
  for i = 1 : ntlist          ## t list loop (db to match against)
    P = load(tlist{i});
