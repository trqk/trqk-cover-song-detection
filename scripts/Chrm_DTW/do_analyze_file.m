function opfile = do_analyze_file(infile, srcprepend, srcext, dstprepend, dstext, skip, fctr, fsd, ctype, take1, pre_proc)
% do_analyze_file(listfile,srcprepend,srcext,dstprepend,dstext,fctr,fsd,ctype)
...
##take1 = false;   % take 1 minute from the middle of the song and pre-process the c-gram (power, norm, fft)
##pre_proc = false;     % do the pre-processing power/norm/FFT before the save

% preallocate array to placate mlint
opfile = '';

if length(srcext) > 0
  if infile(end - length(srcext) + 1 : end) == srcext
    % chop off srcext already there
    infile = infile(1 : end - length(srcext));
  end
else
  ...
  infile = fullfile(srcpath, srcname);
end
...
if exist (ofname, "file")     # if the file exists, read it...
  fnd = 1;
%    disp(['Song ', num2str(songn), ' - ', infile, ' - ChrmGrm file found']);
elseif songn > skip
  disp(['Song ', num2str(songn), ' = ', infile]);
...
%      ' bpm=', num2str(60/median(diff(bts)))]);
  ...
  disp([datestr(rem(now,1), '    HH:MM:SS - '), ofname, ' - nbeats = ', num2str(size(F, 2))]);
end
opfile = ofname;
